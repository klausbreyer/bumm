...
export interface Project { version: 1; pack: PackId; name: string; bpm: number; mix: Mix; song: SongPart[] }
export interface Studio { version: 1; currentPack: PackId; projects: Record<PackId, Project> }
...
export function makeProject(pack: PackId): Project {
  ...
  return {
    version: 1, pack, name: PACKS[pack].projectName, bpm: PACKS[pack].bpm, mix,
    song: [
      { id: 'intro', name: 'Intro', bars: 4, mix: intro },
      { id: 'beat', name: 'Beat', bars: 8, mix: beat },
      { id: 'hook', name: 'Hook', bars: 8, mix: clone(mix) },
    ],
  };
}

export function makeStudio(): Studio {
  return { version: 1, currentPack: 'hiphop', projects: { hiphop: makeProject('hiphop'), techno: makeProject('techno') } };
}
...
export function validProject(value: unknown, pack: PackId): value is Project {
  if (!record(value) || value.version !== 1 || value.pack !== pack) return false;
...
export function parseStudio(serialized: string): Studio | null {
  try {
    ...
    if (!record(value) || value.version !== 1 || (value.currentPack !== 'hiphop' && value.currentPack !== 'techno') || !record(value.projects)) return null;
    return validProject(value.projects.hiphop, 'hiphop') && validProject(value.projects.techno, 'techno') ? value as unknown as Studio : null;
  } catch { return null; }
}
