...
export function vocalWorkspace(project: Project, state: Workspace): string {
  const { session, locked, waves, cursor } = state;
  ...
  const timeline = `<div class="vocal-scroll" tabindex="0" role="region" aria-label="Beats und Stimme auf der Zeitleiste"><div class="song-timeline" style="min-width:${72 + bars * 32}px" data-duration="${seconds}">
    ...
    <button class="timeline-label voice-track-button" data-action="open-voice" aria-label="Aufnahme öffnen" data-focus="open-voice" ${off(locked)}>Stimme ＋</button><div class="voice-lane" style="height:${Math.max(1, rowEnds.length) * 82 + 16}px">${voices || `<button class="empty-lane empty-voice" data-action="open-voice" ${off(locked)}>＋ Stimme aufnehmen</button>`}</div>
    <div class="playhead-area" aria-hidden="true"><div id="song-playhead" style="left:${percent(cursor)}%"><span>▼</span></div></div>
  </div></div>`;
  return timeline;
}

export function recordingPanel(project: Project, state: Workspace): string {
  const { session, locked, headphones, cursor } = state;
  const selected = project.vocals.find(clip => clip.id === state.selectedVocalId);
  if (selected && !state.voiceSetup && !session) {
    return `<section class="voice-recorder" aria-label="Aufnahme bearbeiten">
      <div class="recording-edit-actions"><button class="action-button" data-action="prepare-replacement" data-focus="prepare-replacement" ${off(locked)}>Neu aufnehmen</button><button class="action-button" data-action="remove-vocal" ${off(locked)}>Aufnahme löschen</button></div>
      ${voiceMix(project, selected.id, locked)}
    <details data-panel="clip" class="settings-panel"><summary>Aufnahme anpassen</summary><div class="voice-timing"><label><span>Früher</span><input type="range" min="-250" max="250" step="10" value="${selected.shiftMs}" data-voice-control="timing" data-focus="voice-timing" aria-label="Stimme zeitlich verschieben" ${off(locked)}><span>Später</span><output id="timing-value">${selected.shiftMs} ms</output></label><p>Falls deine Aufnahme etwas neben dem Beat sitzt.</p></div><div class="clip-position"><label>Start (Sekunden)<input type="number" min="0" max="${MAX_START_SECONDS}" step="0.1" value="${selected.startSeconds}" data-clip-position="startSeconds" data-focus="clip-start" ${off(locked)}></label><label>Länge (Sekunden)<input type="number" min="0.1" max="${MAX_TAKE_SECONDS}" step="0.1" value="${selected.durationSeconds}" data-clip-position="durationSeconds" data-focus="clip-duration" ${off(locked)}></label></div></details>
    </section>`;
  }
  const finishing = session?.phase === 'finishing' || session?.phase === 'saving';
  const buttonText = !session ? 'Aufnahme starten'
  ...
  return `<section class="voice-recorder" aria-label="Gesangsaufnahme">${state.microphone}
    <div class="headphone-choice"><strong>Trägst du Kopfhörer?</strong><div class="headphone-options"><label><input type="radio" name="headphones" value="no" ${!headphones ? 'checked' : ''} ${off(locked)}> Nein</label><label><input type="radio" name="headphones" value="yes" ${headphones ? 'checked' : ''} ${off(locked)}> Ja</label></div><p>${headphones ? 'Du hörst den Beat beim Aufnehmen über Kopfhörer.' : 'Beim Aufnehmen bleibt es stumm. Du siehst den Takt.'}</p></div>
    <p id="record-status" role="status">${session ? session.phase === 'preparing' ? 'Erlaube den Mikrofonzugriff. Dann geht es los.' : 'Gleich geht es los.' : `Ab ${time(targetSeconds)} · vier Schläge einzählen, dann singen. Stopp, wenn du fertig bist. Max. 2 Minuten.`}</p>
    ${!selected && project.vocals.length >= MAX_VOCALS ? '<p>Acht Aufnahmen sind im Song. Wähle eine zum Ersetzen oder Löschen.</p>' : ''}
    <div class="record-controls">${session ? `<button class="action-button" data-action="cancel-record" ${off(finishing)}>Abbrechen</button>` : selected ? `<button class="action-button" data-action="cancel-setup" ${off(locked)}>Zurück</button>` : ''}<button class="record-button" data-action="record" data-focus="record" ${off(finishing || (!session && (locked || state.microphoneUnavailable || (!selected && project.vocals.length >= MAX_VOCALS))) || session?.phase === 'preparing' || session?.phase === 'count-in')}><span aria-hidden="true">●</span><span>${buttonText}</span></button></div>
    ${session ? '<div class="record-progress"><div id="record-progress-fill"></div></div><div class="mic-meter" role="meter" aria-label="Mikrofonpegel" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><span>Mikrofon</span><i><b id="mic-level"></b></i></div>' : ''}
  </section>`;
}

function voiceMix(project: Project, selectedId: string | null, locked: boolean): string {
  const selected = project.vocals.find(clip => clip.id === selectedId);
  if (!selected) return '';
  return `<section class="section-mix" aria-label="Mix dieser Aufnahme"><h3>Stimme &amp; Beat mischen</h3><p>Nur für diese Aufnahme · ${time(selected.startSeconds)} bis ${time(selected.startSeconds + selected.durationSeconds)}</p><div class="voice-mix-controls"><label>Beat<input type="range" min="0" max="100" value="${Math.round((selected.beatLevel ?? project.beatLevel) * 100)}" data-voice-control="beat" aria-label="Beat Lautstärke" data-focus="beat-level" ${off(locked)}></label><label>Meine Stimme<input type="range" min="0" max="100" value="${Math.round((selected?.volume ?? .9) * 100)}" data-voice-control="volume" aria-label="Stimme Lautstärke" data-focus="voice-level" ${off(locked || !selected)}></label></div></section>`;
}
